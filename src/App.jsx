...
export default function App() {
    ...
    for (const symbol of eligiblePairs) {
      ...
      if (existing&&existing.side!==sig.action) {
        ...
        if(paperMode||isDemo){
          ...
          // Live mode — real close order placed; re-fetch real balance after settlement
          setTimeout(()=>fetchBalances(), 2500);
        }
      ...
      if (!existing||(existing&&existing.side!==sig.action)) {
        ...
        const usdtBal = parseFloat(
          balancesRef.current.find(b=>b.asset==="USDT")?.free||"0"
        );
  ...
  },[cfg,eligiblePairs,fetchKlinesAndAnalyze,placeOrder,addLog,fetchBalances,paperMode,isDemo]);
  ...
  },[placeOrder,addLog,fetchBalances]);
