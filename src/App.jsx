...
export default function App() {
  ...
  const runBotTick = useCallback(async()=>{
    addLog("Scanner",`[${isDemo?"DEMO":"LIVE"}] Scanning ${eligiblePairs.length} pairs on ${cfg.interval} · ATR SL/TP enabled · Min conf ${cfg.minConfidence}%`,"info");
    for (const symbol of eligiblePairs) {
      ...
      if (!existing||(existing&&existing.side!==sig.action)) {
        // Use exactly the configured amount; cap only if balance is genuinely insufficient
        const usdtBal = parseFloat(
          balances.find(b=>b.asset==="USDT")?.free||"0"
        );
        const requestedAmt = parseFloat(cfg.amount)||1;
        // Allow any amount ≥ $0.10 — placeOrder auto-bumps to exchange minimum if needed
  ...
  },[cfg,eligiblePairs,fetchKlinesAndAnalyze,placeOrder,addLog,balances,paperMode,isDemo]);
