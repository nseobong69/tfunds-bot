...
const BASE_BB = "https://api.bybit.com";
const BASE_OKX= "https://www.okx.com";
const BASE_KC = "https://api.kucoin.com";
...
async function bbSign(apiKey, apiSecret, path, params={}, method="GET", body=null) {
  ...
  const res = await fetch(`${BASE_BB}${path}${qs}`,{method,headers:hdrs,body:body?JSON.stringify(body):null});
...
async function okxSign(apiKey, apiSecret, passphrase, path, method="GET", body=null) {
  ...
  const res = await fetch(`${BASE_OKX}${path}`,{method,
...
async function kcSign(apiKey, apiSecret, passphrase, path, method="GET", body=null) {
  ...
  const res = await fetch(`${BASE_KC}${path}`,{method,
...
function SetupScreen({ onConnect, onDemo, tradeAmount, setTradeAmount }) {
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      setError(
        e.message.includes("401")||e.message.includes("403")||e.message.includes("10003")||e.message.includes("400100")
          ? "Invalid API credentials — check your key, secret"+(needsPass?" and passphrase":"")+". "
...
export default function App() {
  ...
  useEffect(()=>{ validSymbolsRef.current = null; instrInfoRef.current = {}; },[creds]);
        ...
        setTimeout(()=>{
          setBalances(prev=>{
            ...
            if (usdtFree > 0 && usdtFree < 5) {
              addLog("Account",`⚠ Low balance: $${usdtFree.toFixed(4)} USDT — bot will skip trades below exchange minimums (~$1/order). Add funds for reliable trading.`,"warn");
            }
            return prev;
          });
        }, 500);
  ...
  const placeOrder = useCallback(async(symbol,side,quantity)=>{
    ...
    try {
      ...
      if (ex==="binance") {
        ...
        // Enforce minimum order value (~$1 floor)
        const price = priceRef.current[symbol]||0;
        const orderValue = adjQty * price;
        if (orderValue < minOrderAmt) {
          addLog("Executor",`Order skipped [bybit]: ${symbol} value $${orderValue.toFixed(4)} < min $${minOrderAmt}`,"warn");
          return null;
        }
        ...
        // ── Update local balance display so user can see it working ──
        const orderCost = adjQty * price;
        if (side==="BUY") {
          setBalances(prev=>prev.map(b=>
            b.asset==="USDT"
              ? {...b, free: String(Math.max(0, parseFloat(b.free||0) - orderCost).toFixed(4))}
              : b
          ));
        } else {
          // On SELL, the USDT comes back (approximate — exchange will settle the real amount)
          setBalances(prev=>prev.map(b=>
            b.asset==="USDT"
              ? {...b, free: String((parseFloat(b.free||0) + orderCost).toFixed(4))}
              : b
          ));
        }
      } else if (ex==="okx") {
  ...
  const runBotTick = useCallback(async()=>{
    addLog("Scanner",`[${isDemo?"DEMO":"LIVE"}] Scanning ${cfg.pairs.length} pairs on ${cfg.interval} · ATR SL/TP enabled · Min conf ${cfg.minConfidence}%`,"info");
    for (const symbol of cfg.pairs) {
      // Skip symbols the exchange doesn't support (prevents "Not supported symbols" errors)
      ...
      if (existing&&existing.side!==sig.action) {
        ...
        if(paperMode||isDemo){
          ...
          // Live mode — capital returns + PnL
          const dollarPnl=(pnlPct/100)*(existing.qty*existing.entry);
          const capital=existing.qty*existing.entry;
          setBalances(prev=>prev.map(b=>b.asset==="USDT"
            ?{...b,free:String((parseFloat(b.free||0)+capital+dollarPnl).toFixed(4))}:b));
        }
      ...
      if (!existing||(existing&&existing.side!==sig.action)) {
        // Cap trade amount to available USDT balance (with 2% safety buffer)
        const usdtBal = parseFloat(
          balances.find(b=>b.asset==="USDT")?.free||"0"
        );
        const requestedAmt = parseFloat(cfg.amount)||10;
        ...
        const qty=+(safeAmt/price).toFixed(6);
        ...
        if (!res) continue;
        // For live mode: deduct balance immediately so UI reflects the spend
        if (!paperMode && !isDemo && sig.action==="BUY") {
          const spent = qty * price;
          setBalances(prev=>prev.map(b=>
            b.asset==="USDT"
              ? {...b, free: String(Math.max(0, parseFloat(b.free||0) - spent).toFixed(4))}
              : b
          ));
        }
        const pos={
  ...
  },[cfg,fetchKlinesAndAnalyze,placeOrder,addLog,balances,paperMode,isDemo]);
    ...
    posRef.current.forEach(async pos=>{
      ...
      if (hitSL||hitTP) {
        ...
        if(paperMode||isDemo){
          ...
          // Live mode — return the capital + PnL to the display balance
          const dollarPnl=(pnlPct/100)*(pos.qty*pos.entry);
          const capital = pos.qty*pos.entry;
          setBalances(prev=>prev.map(b=>b.asset==="USDT"
            ?{...b,free:String((parseFloat(b.free||0)+capital+dollarPnl).toFixed(4))}:b));
          addLog("Balance",`Balance updated: ${pnlPct>=0?"+"+"$"+(dollarPnl.toFixed(4)):"-$"+(Math.abs(dollarPnl).toFixed(4))} P&L from ${pos.symbol}`,"ok");
        }
  ...
  if (!creds&&!isDemo) return (
    <SetupScreen
      ...
      onConnect={c=>{ setCreds(c); setIsDemo(false); setCfg(x=>({...x,amount:tradeAmount})); }}
      onDemo={()=>{ setCfg(x=>({...x,amount:tradeAmount}));
        setIsDemo(true);
        setPaperMode(true);
  ...
  return (
    <div style={{background:"#020810",minHeight:"100vh",fontFamily:"'JetBrains Mono','Courier New',monospace",
      ...
      <div style={{position:"fixed",bottom:70,right:18,zIndex:150,display:"flex",flexDirection:"column",alignItems:"flex-end",gap:8}}>
        ...
        <button
          onClick={()=>{ if(botRunning){ setShowStopModal(true); } else { setBotRunning(true); }}}
          style={{
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.82)",zIndex:200,
          ...
          <div style={{background:"#0a1628",border:"1px solid rgba(239,68,68,.3)",
            ...
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              ...
              <button onClick={async ()=>{
                ...
                if (closedNow.length>0) {
                  ...
                  if (isDemo||paperMode) {
                    ...
                    // Live — real orders placed; re-fetch balance after settlement
                    posRef.current.forEach(pos=>{
                      const closeSide = pos.side==="BUY"?"SELL":"BUY";
                      placeOrder(pos.symbol,closeSide,pos.qty);
                    });
                    setTimeout(()=>fetchBalances(), 2500);
                    addLog("Bot",
                      `All ${closedNow.length} position${closedNow.length!==1?"s":""} closed — Net P&L: ${totalDollarPnl>=0?"+":""}$${totalDollarPnl.toFixed(2)} — balance updating from exchange`,
                      totalDollarPnl>=0?"ok":"warn");
                  }
                }
                setBotRunning(false);
              ...
              <button onClick={async ()=>{
                // For demo/paper: credit balance first, then open wallet tab
                if (isDemo||paperMode) {
                  const currentPrices = priceRef.current;
              ...
              <button onClick={async ()=>{
                ...
                posRef.current.forEach(pos => {
                  ...
                  totalDollarPnl += (pnlPct / 100) * (pos.qty * pos.entry);
                  closedNow.push({...pos, closePrice:price, pnl:pnlPct, closeTs:new Date(), result:"STOPPED"});
                  placeOrder(pos.symbol, pos.side==="BUY"?"SELL":"BUY", pos.qty);
                  addLog("Bot", `Force-closed ${pos.symbol} on STOP — PnL: ${fmtP(pnlPct)}`, pnlPct>=0?"ok":"warn");
                });
                if (closedNow.length > 0) {
                  setTrades(prev => [...closedNow, ...prev.slice(0, 199)]);
