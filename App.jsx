...
const BASE_CB = "https://api.coinbase.com";
...
async function cbSign(apiKey, apiSecret, path, method="GET", body=null) {
  ...
  const res = await fetch(`${BASE_CB}${path}`, {
...
const TOP_PAIRS = [
  ...
  "UNIUSDT","ATOMUSDT","LTCUSDT","NEARUSDT","APTUSDT","ARBUSDT",
...
function SetupScreen({ onConnect, onDemo }) {
  ...
  const handleConnect = async () => {
    ...
    try {
      if (exchange==="binance") {
        const data = await bSign(apiKey.trim(), apiSecret.trim(), {}, "/api/v3/account");
        onConnect({ exchange, apiKey:apiKey.trim(), apiSecret:apiSecret.trim(), accountData:data });
      } else {
        const data = await cbSign(apiKey.trim(), apiSecret.trim(), "/api/v3/brokerage/accounts");
        onConnect({ exchange, apiKey:apiKey.trim(), apiSecret:apiSecret.trim(), accountData:data });
      }
    } catch(e) {
      setError(
        e.message.includes("401")||e.message.includes("403")
          ? "Invalid API credentials — check your key & secret."
          : e.message.includes("fetch")
            ? "Network error. Ensure API key has no IP restriction, or use a local proxy."
            : e.message
      );
    ...
    <div style={{minHeight:"100vh",background:"#020810",display:"flex",alignItems:"center",
      ...
      <div style={{width:"100%",maxWidth:460,padding:24,position:"relative",zIndex:2}}>
        ...
        <div style={{textAlign:"center",marginBottom:36}}>
          ...
          <div style={{fontSize:10,color:"rgba(100,120,160,.5)",letterSpacing:1,lineHeight:1.8}}>
            Real TA engine · Live execution · 8 indicators<br/>
            <span style={{color:"rgba(0,245,196,.4)"}}>EMA · RSI · MACD · BB · ATR · ADX · Volume · Patterns</span>
          </div>
        </div>
          ...
          <div style={{display:"flex",gap:1}}>
            {["binance","coinbase"].map(ex=>(
              <button key={ex} className={`exbtn ${exchange===ex?"sel":""}`}
                style={{borderRadius:ex==="binance"?"3px 0 0 3px":"0 3px 3px 0"}}
        ...
        <button className="dbtn" onClick={onDemo}>◈  DEMO MODE — No API key needed</button>
...
export default function App() {
  ...
  const fetchBalances = useCallback(async()=>{
    ...
    try {
      if (creds.exchange==="binance") {
        const data = await bSign(creds.apiKey,creds.apiSecret,{},"/api/v3/account");
        setBalances((data.balances||[]).filter(b=>parseFloat(b.free)>0||parseFloat(b.locked)>0));
        addLog("Account","Balances loaded from Binance","ok");
      } else {
        const data = await cbSign(creds.apiKey,creds.apiSecret,"/api/v3/brokerage/accounts");
        ...
        addLog("Account","Balances loaded from Coinbase","ok");
      }
  ...
  const fetchKlinesAndAnalyze = useCallback(async(symbol)=>{
    ...
    try {
      // Fetch 150 candles for robust indicator warmup
      const data = await bPublic("/api/v3/klines",{symbol,interval:cfg.interval,limit:"150"});
      ...
      const sig     = generateSignal(closes, highs, lows, opens, volumes);
      setKlines(prev=>({...prev,[symbol]:{closes,highs,lows,opens,volumes,data}}));
      setSignals(prev=>({...prev,[symbol]:sig}));
      return sig;
    } catch(e) {
      addLog("TA",`Kline fetch failed for ${symbol}: ${e.message}`,"error");
      return null;
    }
  },[cfg.interval,isDemo,addLog]);
  ...
  const fetchPrices = useCallback(async()=>{
    ...
    try {
      ...
      priceRef.current=map;
      setPrices(map);
    } catch(e){ addLog("Market","Price fetch error","error"); }
  },[isDemo,addLog]);
  ...
  const placeOrder = useCallback(async(symbol,side,quantity)=>{
    ...
    try {
      const data = await bSign(creds.apiKey,creds.apiSecret,
        {symbol,side,type:"MARKET",quantity},"/api/v3/order","POST");
      addLog("Executor",`✓ Real order filled: ${side} ${symbol} #${data.orderId}`,"ok");
      return data;
    } catch(e) {
      addLog("Executor","Order failed: "+e.message,"error");
      return null;
    }
  },[paperMode,isDemo,creds,addLog]);

  /* ── Bot tick — TA-driven with ATR SL/TP ── */
  const runBotTick = useCallback(async()=>{
    addLog("Scanner",`Scanning ${cfg.pairs.length} pairs on ${cfg.interval} · ATR SL/TP enabled`,"info");
    for (const symbol of cfg.pairs) {
      ...
      if (!existing||(existing&&existing.side!==sig.action)) {
        const qty=+(parseFloat(cfg.amount)/price).toFixed(6);
        // ATR-based SL/TP if available, fallback to config %
        ...
        const res = await placeOrder(symbol,sig.action,qty);
        if (!res) continue;
        const pos={
          ...
          openTs:new Date(), rsi:sig.meta?.rsi, macd:sig.meta?.macd,
        };
        setOpenPos(prev=>[...prev.filter(p=>p.symbol!==symbol),pos]);
        addLog("Bot",
          `Opened ${sig.action==="BUY"?"LONG":"SHORT"} ${symbol} | Conf:${sig.confidence}% | ${sig.reasons[0]} | SL:${slPct.toFixed(2)}% TP:${tpPct.toFixed(2)}%`,
          "signal"
        );
      }
    }
  },[cfg,fetchKlinesAndAnalyze,placeOrder,addLog]);
  ...
  const monitorPositions = useCallback(()=>{
    posRef.current.forEach(async pos=>{
      ...
      if (hitSL||hitTP) {
        ...
        addLog("Risk",`${pos.symbol} hit ${result} @ $${priceFmt(price)} — PnL: ${fmtP(pnlPct)}`,hitTP?"ok":"warn");
      }
    });
  },[placeOrder,addLog]);
  ...
  /* ── Binance Withdrawal (via user's own API key) ── */
  const handleWithdrawal = useCallback(async () => {
    ...
    if (isDemo || paperMode) {
      setWdStatus({type:"ok",msg:`[SIMULATED] ${wdForm.amount} ${wdForm.coin} withdrawal to ${wdForm.address.slice(0,10)}... queued`});
      return;
    }
    if (!creds) { setWdStatus({type:"error",msg:"Connect your Binance API key first"}); return; }
    try {
      setWdStatus({type:"info",msg:"Submitting to Binance..."});
      await bSign(creds.apiKey, creds.apiSecret, {
        coin:    wdForm.coin,
        network: wdForm.network,
        address: wdForm.address.trim(),
        amount:  wdForm.amount,
      }, "/sapi/v1/capital/withdraw/apply", "POST");
      setWdStatus({type:"ok",msg:`✓ Withdrawal of ${wdForm.amount} ${wdForm.coin} submitted. Check Binance for status.`});
      setWdForm(f=>({...f, address:"", amount:""}));
    } catch(e) { setWdStatus({type:"error",msg:"Binance error: "+e.message}); }
  }, [wdForm, creds, isDemo, paperMode]);
  ...
  useEffect(()=>{
    if (botRunning) {
      runBotTick();
      botRef.current=setInterval(runBotTick,60000);
  ...
  if (!creds&&!isDemo) return (
    <SetupScreen
      onConnect={c=>{ setCreds(c); setIsDemo(false); }}
      onDemo={()=>{ setIsDemo(true); addLog("System","Demo mode active — paper orders only","ok"); }}
    />
  );
      ...
      <div style={{background:"rgba(1,4,12,.99)",borderBottom:"1px solid rgba(0,245,196,.09)",
        ...
        {!isDemo&&(
          <div style={{display:"flex"}}>
            ...
            <button onClick={()=>{if(!botRunning)setPaperMode(false);}} style={{fontFamily:"Orbitron",fontSize:7.5,
        ...
        {!paperMode&&!isDemo&&<span className="bdg bw" style={{fontSize:6.5}}>⚠ REAL ORDERS</span>}
        {isDemo&&<span className="bdg bw" style={{fontSize:6.5}}>DEMO</span>}
        <div style={{display:"flex",gap:1,marginLeft:6}}>
          {["dashboard","signals","positions","history","wallet","config"].map(t=>(
        ...
        <button className={`sbtn ${botRunning?"sst":"sgo"}`}
          ...
          <div style={{flex:1,overflowY:"auto",padding:16}} className="scr">
            {posWithPnl.length===0
              ?<div style={{textAlign:"center",padding:70,fontSize:10,
                ...
                <span style={{fontSize:8,color:"rgba(100,120,160,.18)"}}>Start the bot to begin trading</span>
              </div>
              :posWithPnl.map(p=>(
                <div key={p.id} style={{background:"rgba(3,9,22,.95)",
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8,marginBottom:10}}>
                    {[
                      ...
                      ["Stop Loss",     `$${priceFmt(p.sl)} (${p.slPct?.toFixed(2)||"—"}%)`],
            ...
            <div style={{maxWidth:600}}>
              ...
              <div style={{marginBottom:20}}>
                <div style={{fontFamily:"Orbitron",fontSize:8,letterSpacing:2,
                  color:"rgba(0,245,196,.5)",marginBottom:10}}>TRADING PAIRS (max 8)</div>
                      ...
                      <button key={p} className={`pchip ${cfg.pairs.includes(p)?"sel":""}`}
                        ...
                        onClick={()=>setCfg(c=>({...c,pairs:
                          c.pairs.includes(p)
                            ...
                            :c.pairs.length<8?[...c.pairs,p]:c.pairs}))}>
                        {p.replace("USDT","")}
              ...
              <div style={{background:"rgba(0,245,196,.04)",border:"1px solid rgba(0,245,196,.14)",
                ...
                <div>• Fetches <b>150 real {cfg.interval} OHLCV candles</b> from Binance for each pair</div>
          ...
          <div style={{flex:1,overflowY:"auto",padding:20}} className="scr">
            <div style={{maxWidth:640,margin:"0 auto"}}>
              ...
              <div style={{background:"rgba(3,9,22,.95)",border:"1px solid rgba(0,245,196,.12)",
                borderRadius:4,padding:18,marginBottom:14}}>
                <div style={{fontFamily:"Orbitron",fontSize:8,letterSpacing:2,
                  color:"rgba(0,245,196,.5)",marginBottom:4}}>BINANCE WITHDRAWAL</div>
                <div style={{fontSize:9,color:"#506070",marginBottom:14,lineHeight:1.6}}>
                  Withdraws from your own Binance account using your API key.
                  Binance handles the transfer — TFunds only submits the request.
                </div>
                ...
                <div style={{marginTop:10,fontSize:8.5,color:"rgba(100,120,160,.35)",lineHeight:1.6}}>
                  ⚠ Your Binance API key needs <b>withdrawal permission enabled</b> for this to work.
                  In Demo/Paper mode this is simulated only.
                </div>
              </div>

              {/* ── BINANCE PAY ── */}
              <div style={{background:"rgba(3,9,22,.95)",border:"1px solid rgba(245,184,0,.14)",
                borderRadius:4,padding:18,marginBottom:14}}>
                <div style={{fontFamily:"Orbitron",fontSize:8,letterSpacing:2,
                  color:"rgba(245,184,0,.7)",marginBottom:4}}>BINANCE PAY</div>
                <div style={{fontSize:9.5,color:"#6b7a99",lineHeight:1.7,marginBottom:16}}>
                  Send a payment via Binance Pay — accepted in any crypto Binance supports.
                  Payments go directly to the TFunds merchant account. Binance handles everything.
                </div>
                <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:16}}>
                  {[["Zero fees","No transaction fees for sender"],
                    ["Any crypto","BTC, ETH, BNB, USDT, 70+ coins"],
                    ["Instant","Settles immediately on Binance"]
                  ].map(([t,d])=>(
                    <div key={t} className="ibox" style={{border:"1px solid rgba(245,184,0,.1)"}}>
                      <div style={{fontFamily:"Orbitron",fontSize:7.5,color:"#f5b800",marginBottom:4}}>{t}</div>
                      <div style={{fontSize:8.5,color:"#6b7a99",lineHeight:1.5}}>{d}</div>
                    </div>
                  ))}
                </div>
                <a href={`https://pay.binance.com/en/checkout/${BINANCE_PAY_MERCHANT_ID}`}
                  target="_blank" rel="noopener noreferrer"
                  style={{display:"block",width:"100%",padding:"13px",textAlign:"center",
                    fontFamily:"Orbitron",fontWeight:700,fontSize:10,letterSpacing:2,
                    background:"linear-gradient(135deg,#f5b800,#f59e0b)",color:"#020810",
                    borderRadius:3,textDecoration:"none",cursor:"pointer",
                    boxShadow:"0 0 20px rgba(245,184,0,.2)"}}>
                  💛 PAY WITH BINANCE PAY
                </a>
                {BINANCE_PAY_MERCHANT_ID==="YOUR_BINANCE_PAY_MERCHANT_ID"&&(
                  <div style={{marginTop:10,fontSize:8.5,color:"rgba(245,158,11,.55)",lineHeight:1.6}}>
                    ⚠ Placeholder ID active — apply for your free Merchant ID at binance.com → Pay → Merchant
                  </div>
                )}
              </div>
              ...
              <div style={{background:"rgba(0,0,0,.25)",border:"1px solid rgba(0,245,196,.07)",
                ...
                <div>• Wallet connection is read-only for balance display</div>
                <div>• Withdrawals use your own Binance API key, processed by Binance directly</div>
                <div>• Each user connects their own accounts — no shared funds</div>
              </div>
