...
const TOP_PAIRS = [
  ...
  "SUIUSDT","INJUSDT","SEIUSDT","TIAUSDT","OPUSDT","LDOUSDT",
  "FETUSDT","RENDERUSDT","WLDUSDT","STXUSDT","RUNEUSDT","FILUSDT",
  "IMXUSDT","GRTUSDT","SANDUSDT","MANAUSDT","AXSUSDT","APEUSDT",
  "GALAUSDT","FTMUSDT","ALGOUSDT","VETUSDT","ICPUSDT","HBARUSDT",
  "QNTUSDT","FLOWUSDT","XTZUSDT","AAVEUSDT","MKRUSDT","SNXUSDT",
  "CRVUSDT","COMPUSDT","YFIUSDT","SUSHIUSDT","1INCHUSDT","BALUSDT",
  "CELOUSDT","KSMUSDT","IOTAUSDT","ZILUSDT","ONTUSDT","WAVESUSDT",
  "RVNUSDT","ZENUSDT","ANKRUSDT","CELRUSDT","BANDUSDT","COTIUSDT",
  "TRXUSDT","XLMUSDT","BCHUSDT","ETCUSDT","EOSUSDT","DASHUSDT",
  "ZECUSDT","XMRUSDT","NEOUSDT","KAVAUSDT","CHZUSDT","HOTUSDT",
  "ENJUSDT","BATUSDT","ZRXUSDT","STORJUSDT","OCEANUSDT","SXPUSDT"
];
...
export default function App() {
  ...
  const fetchBalances = useCallback(async()=>{
    ...
    try {
      if (ex==="binance") {
        ...
        const data = await bbSign(k,s,"/v5/account/wallet-balance",{accountType:"UNIFIED"});
  ...
  const fetchPrices = useCallback(async()=>{
    ...
    } catch(e){ addLog("Market",`Price fetch error [${ex}]: ${e.message}`,"error"); }
  },[isDemo,creds,addLog]);
    ...
    try {
      ...
      if (ex==="binance") {
        ...
        data = await bbSign(k,s,"/v5/order/create",{},
          "POST",{category:"spot",symbol,side:side==="BUY"?"Buy":"Sell",orderType:"Market",qty});
        addLog("Executor",`✓ Bybit order: ${side} ${symbol} #${data.orderId}`,"ok");
      } else if (ex==="okx") {
        const instId = toExSymbol(symbol,"okx");
      ...
      <div style={{position:"fixed",bottom:70,right:18,zIndex:150,display:"flex",flexDirection:"column",alignItems:"flex-end",gap:8}}>
        ...
        {usdtBal&&(
          <div style={{background:"rgba(1,4,12,.95)",border:"1px solid rgba(0,245,196,.2)",
            ...
            <span style={{fontFamily:"Orbitron",fontWeight:700,fontSize:11,color:"#00f5c4"}}>${fmt(usdtBal.free)}</span>
          </div>
        )}
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.82)",zIndex:200,
          ...
          <div style={{background:"#0a1628",border:"1px solid rgba(239,68,68,.3)",
            ...
            <div style={{fontSize:9.5,color:"rgba(160,180,210,.5)",marginBottom:24,letterSpacing:1}}>
              {isDemo?"DEMO MODE":"LIVE MODE"} · {openPos.length} open position{openPos.length!==1?"s":""} will be closed at market price
            </div>
            ...
            <div style={{background:"rgba(0,245,196,.04)",border:"1px solid rgba(0,245,196,.1)",
              ...
              {[
                ...
                ["BALANCE",        isDemo||paperMode?`$${fmt(balances.find(b=>b.asset==="USDT")?.free||"10000")}`:"See exchange"],
              ].map(([label,val])=>(
                <div key={label}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
                ...
                <button onClick={()=>{
                  ...
                  <div style={{textAlign:"left"}}>
                    ...
                    <div style={{fontSize:7.5,color:"rgba(0,245,196,.5)",fontWeight:400,marginTop:2,letterSpacing:1}}>
                      Stops bot and opens withdrawal panel
                    </div>
                  </div>
                </button>
              )}
              ...
              <button onClick={async ()=>{
                // Close all open positions at current market price and settle balance
                ...
                posRef.current.forEach(pos => {
                  ...
                  const pnlPct = pos.side === "BUY"
                    ...
                    : (pos.entry - price) / pos.entry * 100;
                  const dollarPnl = (pnlPct / 100) * (pos.qty * pos.entry);
                  const capitalReturned = pos.qty * pos.entry; // original cost
                  ...
                  // Place simulated sell order for demo/paper; real sell for live
                  const closeSide = pos.side === "BUY" ? "SELL" : "BUY";
                ...
                if (closedNow.length > 0) {
                  // Add all closed trades to history
                  setTrades(prev => [...closedNow, ...prev.slice(0, 199)]);
                  // Clear all open positions
                  setOpenPos([]);
                  // Update USDT balance: return capital + profit/loss for demo & paper (live handled by exchange)
                  if (isDemo || paperMode) {
